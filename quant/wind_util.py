...
def fill_close_price(security):
    data = w.wsd(security.code, 'close', security.begin_date, security.end_date)
    seri = pd.Series(data.Data[0], index=data.Times)
    seri.name = security.code
    security.close_price_seri = seri

def get_close_price(code, begin_date, end_date, adj_direction='B'):
    adj_param = 'PriceAdj={0}'.format(adj_direction)
    data = w.wsd(code, 'close', begin_date, end_date, adj_param)
    seri = pd.Series(data.Data[0], index=data.Times)
    seri.name = code
    return seri
